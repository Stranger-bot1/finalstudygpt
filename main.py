...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Support both /start (message) and callback-based restart
...
async def restart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    # Call start with the original Update so it can access .callback_query.message
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            SELECTING_SECTION_CLASS: [
                CallbackQueryHandler(class_selected, pattern=r"^class_\d+$")
            ]
        },
    ...
    app.run_polling()  # simple polling without manual asyncio
